...
function SidebarItem({ icon: Icon, label, active, onClick, isMobile }) {
  return (
    <div onClick={onClick} style={{
      ...
      background: active ? 'linear-gradient(135deg, #e34e4e 0%, #d83d31 100%)' : 'transparent',
      color: active ? '#fff' : '#8a96a8',
      fontWeight: active ? '600' : '500',
      transition: 'all 0.2s ease',
      marginBottom: isMobile ? '0' : '6px',
      marginRight: isMobile ? '8px' : '0',
      flexShrink: 0,
      boxShadow: active ? '0 4px 12px rgba(226, 55, 68, 0.25)' : 'none',
    }}>
      <div style={{
  ...
  const filtered = CUSTOMERS
    ...
    .sort((a, b) =>
      sortBy === 'rating' ? b.rating - a.rating :
        sortBy === 'jobs' ? b.jobs - a.jobs :
          sortBy === 'spent' ? parseInt(b.spent.replace(/[^0-9]/g, '')) - parseInt(a.spent.replace(/[^0-9]/g, '')) :
        ...
        <div style={{ display: 'flex', gap: '10px', flexDirection: isMobile ? 'column' : 'row', width: isMobile ? '100%' : 'auto' }}>
          ...
          <div style={{ position: 'relative' }}>
            <button
              ...
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', padding: '10px 16px', borderRadius: '10px', border: '1px solid #e5e7eb', backgroundColor: showSort ? '#fff1f2' : '#fff', fontSize: '13px', fontWeight: '600', color: showSort ? '#e23744' : '#6b7280', cursor: 'pointer', width: isMobile ? '100%' : 'auto' }}>
              <Filter size={15} /> Sort: {sortBy.charAt(0).toUpperCase() + sortBy.slice(1)}
            </button>
            {showSort && (
              <div style={{ position: 'absolute', top: 'calc(100% + 6px)', left: 0, background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, boxShadow: '0 8px 24px rgba(0,0,0,0.1)', zIndex: 50, minWidth: 160, overflow: 'hidden' }}>
                {[['name', 'Name (A-Z)'], ['rating', 'Highest Rating'], ['jobs', 'Most Jobs'], ['spent', 'Most Spent']].map(([val, label]) => (
                  <div key={val} onClick={() => { setSortBy(val); setShowSort(false); }}
                    style={{ padding: '10px 16px', fontSize: '13px', fontWeight: sortBy === val ? '700' : '500', color: sortBy === val ? '#e23744' : '#374151', cursor: 'pointer', backgroundColor: sortBy === val ? '#fff1f2' : 'transparent' }}>
                    {label}
                  </div>
                ))}
              </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isTablet ? '1fr' : '300px 1fr', gap: '20px', minHeight: isTablet ? 'auto' : 'calc(100vh - 220px)' }}>
        ...
        <div style={{ backgroundColor: '#fff', borderRadius: '16px', border: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ padding: '14px 20px', borderBottom: '1px solid #e5e7eb', display: 'flex', alignItems: isMobile ? 'flex-start' : 'center', justifyContent: 'space-between', flexDirection: isMobile ? 'column' : 'row', gap: '12px' }}>
            ...
            <button
              onClick={() => {
                const phone = threads.find(m => m.id === active)?.phone || '';
                if (phone) window.open(`tel:${phone}`);
                else showToast('Calling ' + conv.name + '…', '#10b981');
              }}
              style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 14px', borderRadius: '8px', backgroundColor: '#f0fdf4', color: '#166534', border: 'none', cursor: 'pointer', fontSize: '13px', fontWeight: '600' }}>
              <Phone size={14} /> Call
            </button>
          </div>
            ...
            onClick={() => {
              ...
              inp.onchange = () => {
                if (inp.files[0]) {
                  ...
                  document.getElementById('provider-avatar-preview').style.backgroundImage = `url(${url})`;
                  document.getElementById('provider-avatar-preview').innerText = '';
                }
              };
              inp.click();
            }}
            style={{ fontSize: '12px', color: '#e23744', background: 'none', border: 'none', cursor: 'pointer', fontWeight: '600' }}>Change Photo</button>
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', minHeight: '100vh', backgroundColor: '#f8fafc', fontFamily: "'Inter', sans-serif" }}>
      ...
      <aside style={{
        ...
        background: isMobile ? '#0f172a' : 'linear-gradient(180deg, #0f172a 0%, #1e293b 100%)',
        borderRight: isMobile ? 'none' : '1px solid rgba(255,255,255,0.06)',
        borderBottom: isMobile ? '1px solid rgba(255,255,255,0.08)' : 'none',
        padding: isMobile ? '14px 16px' : '20px 12px',
        display: 'flex', flexDirection: 'column',
        position: isMobile ? 'static' : 'fixed',
        top: 0, bottom: 0, left: 0, zIndex: 50, overflowY: 'auto',
        boxShadow: isMobile ? 'none' : '4px 0 24px rgba(0,0,0,0.18)',
      }}>
        {/* Logo */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: isMobile ? '0 0 14px' : '4px 10px 28px' }}>
          <div style={{
            width: 36, height: 36,
            background: 'linear-gradient(135deg, #e23744, #c0192a)',
            borderRadius: '10px',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            color: '#fff',
          ...
          <span style={{ fontSize: '18px', fontWeight: '800', color: '#f1f5f9', fontFamily: "'Outfit', sans-serif", letterSpacing: '-0.3px' }}>
      ...
      <main style={{ marginLeft: isMobile ? 0 : 250, flex: 1, padding: isMobile ? '24px 16px' : '36px 40px', minWidth: 0, width: '100%', boxSizing: 'border-box' }}>
        <header style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'center', gap: '16px', marginBottom: '28px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', width: isMobile ? '100%' : 'auto', overflowX: isMobile ? 'auto' : 'visible' }}>
            <button
              onClick={() => {
                const rows = [['ID', 'Customer', 'Service', 'Status', 'Amount'],
                ...RECENT_JOBS.map(j => [j.id, j.customer, j.service, j.status, j.time])];
                const csv = rows.map(r => r.join(',')).join('\n');
                const a = document.createElement('a');
                a.href = 'data:text/csv,' + encodeURIComponent(csv);
                a.download = 'provider_report.csv';
                a.click();
                showToast('✓ Report exported as CSV!');
              }}
              style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '10px 18px', borderRadius: '10px', border: '1px solid #e5e7eb', backgroundColor: '#fff', fontSize: '13px', fontWeight: '600', color: '#6b7280', cursor: 'pointer', whiteSpace: 'nowrap' }}>
              <Download size={16} /> Export
            </button>
            <button
              onClick={() => {
                const inp = document.createElement('input');
                inp.type = 'file'; inp.accept = '.csv,.xlsx,.json';
                inp.onchange = () => showToast(`✓ File "${inp.files[0]?.name}" imported!`);
                inp.click();
              }}
              style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '10px 18px', borderRadius: '10px', backgroundColor: '#e23744', fontSize: '13px', fontWeight: '600', color: '#fff', border: 'none', cursor: 'pointer', whiteSpace: 'nowrap' }}>
